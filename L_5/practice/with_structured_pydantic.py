...
class Review(BaseModel):
    key_theams: list[str] = Field(description='Write down all the key theams discussed in the review in a list')
    summary: str = Field(description='A Brief summary about the review')
    sentiment:str = Field(description='sentiment about the review Possitive, negative or nutral')
    pros: Optional[list[str]] = Field(default=None,description='write show all the rows inside the list')
    cons:  Optional[list[str]] = Field(default=None,description='write show all the rows inside the list')
